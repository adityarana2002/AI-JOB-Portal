...
const ApplyJob = () => {
    ...
    const loadJob = async () => {
      ...
      try {
        ...
        const [data, applied] = await Promise.all([
          ...
          applicationService.checkApplied(Number(jobId)).catch(() => false),
        ])
        setJob(data)
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (status === 409) setError('You have already applied to this job.')
      else setError('Unable to submit application. Please try again.')
    }
  ...
  return (
    <div className="page">
      ...
      {alreadyApplied ? (
        <div className="card already-applied-banner" style={{ marginTop: '20px' }}>
          <div className="already-applied-banner__icon">
          ...
          <div>
            ...
            <p>You have already submitted an application for this role.</p>
          </div>
          <Link className="button sm" to="/jobseeker/applications">View my applications</Link>
        </div>
      ) : (
